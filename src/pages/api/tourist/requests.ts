...
interface CreateTourRequestResponse {
  success: boolean;
  code: number;
  data: TourRequest;
  message?: string;
...
// GET - List tour requests with search, filter, pagination
// POST - Create a new tour request
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<GetTourRequestsResponse | CreateTourRequestResponse | ErrorResponse>,
) {
  if (req.method === 'GET') {
    try {
      const {
  ...
  if (req.method === 'POST') {
    try {
      ...
      // Validation
      if (!title || !destination || !startDate || !endDate || !budget || !numberOfPeople || !tourType || !description || !touristId) {
        return res.status(400).json({
          success: false,
          code: 400,
          error: 'Bad request',
          message: 'Missing required fields: title, destination, startDate, endDate, budget, numberOfPeople, tourType, description, touristId',
        });
      }
      ...
      if (budget <= 0 || numberOfPeople <= 0) {
        return res.status(400).json({
          success: false,
          code: 400,
          error: 'Bad request',
          message: 'Budget and numberOfPeople must be greater than 0',
        });
      }
      ...
      const docRef = await addDoc(collection(db, 'tourRequests'), tourRequestData);

      const response: CreateTourRequestResponse = {
        success: true,
        code: 201,
        data: {
          id: docRef.id,
      ...
      return res.status(500).json({
        ...
        code: 500,
        error: 'Internal server error',
        message: error.message || 'Failed to create tour request',
      });
    }
  }
